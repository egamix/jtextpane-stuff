...
//                System.out.println(getWordUnderMouse(textPane, e.getX(), e.getY()));
                HighlightUnderMouse(textPane, e.getX(), e.getY());
    ...
    public static String getWordUnderMouse(JTextComponent textComp, int textAreaMouseXLocation,
                                           int textAreaMouseYLocation) {
        Point pt = new Point(textAreaMouseXLocation, textAreaMouseYLocation);
        int pos = textComp.viewToModel2D(pt);
    ...
    public static String HighlightUnderMouse(JTextComponent textComp, int textAreaMouseXLocation,
                                           int textAreaMouseYLocation) {
        Point pt = new Point(textAreaMouseXLocation, textAreaMouseYLocation);
        int pos = textComp.viewToModel2D(pt);
        String word = "";
        ...
        try {
            ...
            Rectangle posconv= null;
            try {
                posconv = (Rectangle) textComp.modelToView2D(pos);
            } catch (BadLocationException e) {
                e.printStackTrace();
            }
            assert posconv != null;
            posconv.grow(endOffs - begOffs, 0);
            if (!posconv.contains(textAreaMouseXLocation, textAreaMouseYLocation)){
                return null;
            }
...
//            int p1 = p0 + "world".length();
            if (!word.contains(" ")) {
                highlighter.addHighlight(begOffs, endOffs, painter);
            }
        }
        catch( BadLocationException ex ) {
            return null;
